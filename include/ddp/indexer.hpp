...
#define DDP_REDUNDANT_RANDOM_ACCESS_ITER_METHODS(Class_Name)                                                           \
  auto operator-=(std::ptrdiff_t n)->Class_Name& { return (*this) += -n; }                                             \
  auto operator[](std::ptrdiff_t n) const->reference { return *((*this) + n); }                                        \
  friend auto operator+(Class_Name it, std::ptrdiff_t n)->Class_Name { return (it += n); }                             \
  friend auto operator+(std::ptrdiff_t n, Class_Name it)->Class_Name { return (it += n); }                             \
  friend auto operator-(Class_Name it, std::ptrdiff_t n)->Class_Name { return (it += -n); }                            \
  friend auto operator<(Class_Name a, Class_Name b)->bool { return (a - b) < 0; }                                      \
  friend auto operator<=(Class_Name a, Class_Name b)->bool { return (a - b) <= 0; }                                    \
  friend auto operator>(Class_Name a, Class_Name b)->bool { return (a - b) > 0; }                                      \
  friend auto operator>=(Class_Name a, Class_Name b)->bool { return (a - b) >= 0; }                                    \
  static_assert(true, "")
...
struct indexer_proxy_t {
  ...
  auto rows() const -> typename indexer_t::row_kind { return m_indexer->rows(m_current_index); }
  auto cols() const -> typename indexer_t::col_kind { return m_indexer->cols(m_current_index); }

  auto max_rows() const -> typename indexer_t::max_row_kind { return m_indexer->max_rows(); }
  auto max_cols() const -> typename indexer_t::max_col_kind { return m_indexer->max_cols(); }

  auto current_index() const -> index_t { return m_current_index; }
  auto to_forward_iterator() const -> indexer_iter_t<Indexer_Ref>;
};
...
struct indexer_iter_t {
  ...
  indexer_iter_t(Indexer_Ptr indexer, index_t current_index, index_t memory_offset)
  ...
  auto operator+=(std::ptrdiff_t n) -> indexer_iter_t& {
    DDP_ASSERT_MSG_ALL_OF(
  ...
  friend auto operator==(indexer_iter_t a, indexer_iter_t b) -> bool {
...
struct begin_end_impl {
  ...
  static auto begin(Idx const& idx) -> iterator { return {&idx, idx.index_begin(), 0}; }
  static auto end(Idx const& idx) -> iterator { return {&idx, idx.index_end(), idx.required_memory()}; }
};
} // namespace _

// clang-format off
template <typename Idx, typename Impl = _::begin_end_impl<Idx>>
auto begin (Idx const& idx)   -> typename Impl::iterator         { return Impl::begin(idx); }
template <typename Idx, typename Impl = _::begin_end_impl<Idx>>
auto end   (Idx const& idx)   -> typename Impl::iterator         { return Impl::end(idx); }
// clang-format on

template <typename Indexer_Ptr>
auto indexer_proxy_t<Indexer_Ptr>::to_forward_iterator() const -> indexer_iter_t<Indexer_Ptr> {
  return {m_indexer, m_current_index, m_memory_offset};
}
...
struct row_concat_indexer_t {
  ...
  auto clone() const -> row_concat_indexer_t { return {m_idx_l.clone(), m_idx_r.clone()}; }

  auto index_begin() const -> index_t { return m_idx_l.index_begin(); }
  auto index_end() const -> index_t { return m_idx_l.index_end(); }

  auto required_memory() const -> index_t { return m_idx_l.required_memory() + m_idx_r.required_memory(); };
  auto stride(index_t t) const -> index_t { return m_idx_l.stride(t) + m_idx_r.stride(t); }
  auto stride_n(index_t t) const -> index_t { return m_idx_l.stride_n(t) + m_idx_r.stride_n(t); }

  auto rows(index_t t) const -> row_kind { return m_idx_l.rows(t) + m_idx_r.rows(t); }
  auto cols(index_t t) const -> col_kind { return m_idx_l.cols(t); }

  auto max_rows() const -> max_row_kind { return m_idx_l.max_rows() + m_idx_r.max_rows(); }
  auto max_cols() const -> max_col_kind { return m_idx_l.max_cols(); }
...
private:
  outer_product_indexer_t(Idx_L l, Idx_R r, index_t required_memory)
      : m_idx_l{DDP_MOVE(l)}, m_idx_r{DDP_MOVE(r)}, m_required_memory{required_memory} {}

public:
  outer_product_indexer_t(Idx_L l, Idx_R r) : m_idx_l{DDP_MOVE(l)}, m_idx_r{DDP_MOVE(r)}, m_required_memory{0} {
    DDP_ASSERT_MSG_ALL_OF(
        ("", m_idx_l.index_begin() == m_idx_r.index_begin()),
        ("", m_idx_l.index_end() == m_idx_r.index_end()));
  ...
  auto clone() const -> outer_product_indexer_t { return {m_idx_l.clone(), m_idx_r.clone(), m_required_memory}; }

  auto index_begin() const -> index_t { return m_idx_l.index_begin(); }
  auto index_end() const -> index_t { return m_idx_l.index_end(); }

  auto required_memory() const -> index_t { return m_required_memory; };
  auto stride(index_t t) const -> index_t { return (this->rows(t) * this->cols(t)).value(); }

  auto rows(index_t t) const -> row_kind { return m_idx_l.rows(t); }
  auto cols(index_t t) const -> col_kind { return m_idx_r.rows(t); }

  auto max_rows() const -> max_row_kind { return m_idx_l.max_rows(); }
  auto max_cols() const -> max_col_kind { return m_idx_r.max_rows(); }
...
struct regular_indexer_t {
  ...
  regular_indexer_t(index_t begin, index_t end, row_kind rows, col_kind cols)
      : m_rows{rows}, m_cols{cols}, m_begin{begin}, m_end{end} {
    DDP_ASSERT(begin < end);
  }

  auto clone() const -> regular_indexer_t { return *this; }

  auto index_begin() const -> index_t { return m_begin; }
  auto index_end() const -> index_t { return m_end; }

  auto required_memory() const -> index_t { return (m_rows * m_cols).value() * (m_end - m_begin); };
  auto stride(index_t) const -> index_t { return (m_rows * m_cols).value(); };
  auto stride_n(index_t, index_t n) const -> index_t { return n * (m_rows * m_cols).value(); };

  auto rows(index_t) const -> row_kind { return m_rows; };
  auto cols(index_t) const -> col_kind { return m_cols; };

  auto max_rows() const -> max_row_kind { return m_rows; };
  auto max_cols() const -> max_col_kind { return m_cols; };
...
struct shift_time_idx_t {
  ...
  auto clone() const -> shift_time_idx_t { return {m_idx.clone(), m_dt}; }
  auto index_begin() const -> index_t { return m_idx.index_begin() - m_dt; }
  auto index_end() const -> index_t { return m_idx.index_end() - m_dt; }

  auto required_memory() const -> index_t { return m_idx.required_memory(); }
  auto stride(index_t t) const -> index_t { return m_idx.stride(t + m_dt); }
  auto stride_n(index_t t, index_t n) const -> index_t { return m_idx.stride_n(t + m_dt, n); }

  auto rows(index_t t) const -> row_kind { return m_idx.rows(t + m_dt); }
  auto cols(index_t t) const -> col_kind { return m_idx.cols(t + m_dt); }

  auto max_rows() const -> max_row_kind { return m_idx.max_rows(); }
  auto max_cols() const -> max_col_kind { return m_idx.max_cols(); }
...
private:
  range_row_filter_t(Indexer idx, index_t range_begin, index_t range_end, index_t required_memory)
      : m_idx{DDP_MOVE(idx)}, m_range_begin{range_begin}, m_range_end{range_end}, m_required_memory{required_memory} {}

public:
  range_row_filter_t(Indexer idx, index_t range_begin, index_t range_end)
  ...
  auto clone() const -> range_row_filter_t { return {m_idx.clone(), m_range_begin, m_range_end, m_required_memory}; }

  auto index_begin() const -> index_t { return m_idx.index_begin(); }
  auto index_end() const -> index_t { return m_idx.index_end(); }

  auto stride(index_t t) const -> index_t { return (rows(t) * cols(t)).value(); }
  auto stride_n(index_t t, index_t n) const -> index_t {
    DDP_ASSERT(n >= 0);
  ...
  auto required_memory() const -> index_t { return m_required_memory; }
  auto unfiltered_rows(index_t t) const -> typename Indexer::row_kind { return m_idx.rows(t); }

  auto rows(index_t t) const -> row_kind {
    return (t >= m_range_begin and t < m_range_end) ? dyn_index{unfiltered_rows(t)} : dyn_index{0};
  }
  auto cols(index_t) const -> col_kind { return {}; }

  auto max_rows() const -> max_row_kind { return m_idx.max_rows(); };
  auto max_cols() const -> max_col_kind { return fix_index<1>{}; };
...
private:
  periodic_row_filter_t(Indexer idx, index_t period, index_t first_offset, index_t required_memory)
      : m_idx{DDP_MOVE(idx)}, m_period{period}, m_first_offset{first_offset}, m_required_memory{required_memory} {}

public:
  periodic_row_filter_t(Indexer idx, index_t period, index_t first_offset)
  ...
  auto clone() const -> periodic_row_filter_t { return {m_idx.clone(), m_period, m_first_offset, m_required_memory}; }
...
struct outer_prod_result {
  ...
  static auto construct(Idx_L idx_l, Idx_R idx_r) -> type { return {DDP_MOVE(idx_l), DDP_MOVE(idx_r)}; }
};
...
struct outer_prod_result<regular_indexer_t<Rows_L, fix_index<1>>, regular_indexer_t<Rows_R, fix_index<1>>> {
  ...
  static auto construct(idx_left_t idx_l, idx_right_t idx_r) -> type {
    DDP_ASSERT_MSG_ALL_OF(
...
template <typename Idx_L, typename Idx_R>
auto outer_prod(Idx_L idx_l, Idx_R idx_r) -> typename outer_prod_result<Idx_L, Idx_R>::type {
  return outer_prod_result<Idx_L, Idx_R>::construct(DDP_MOVE(idx_l), DDP_MOVE(idx_r));
}

template <typename Idx_L, typename Idx_R>
auto row_concat(Idx_L idx_l, Idx_R idx_r) -> row_concat_indexer_t<Idx_L, Idx_R> {
  return {DDP_MOVE(idx_l), DDP_MOVE(idx_r)};
}

template <typename Idx>
auto periodic_row_filter(Idx idx, index_t period, index_t first_offset) -> periodic_row_filter_t<Idx> {
  return {DDP_MOVE(idx), period, first_offset};
}

template <typename Rows, typename Cols>
auto mat_regular_indexer(index_t begin, index_t end, Rows rows, Cols cols) -> regular_indexer_t<Rows, Cols> {
  return {begin, end, rows, cols};
}

template <typename Rows>
auto vec_regular_indexer(index_t begin, index_t end, Rows rows) -> regular_indexer_t<Rows, fix_index<1>> {
