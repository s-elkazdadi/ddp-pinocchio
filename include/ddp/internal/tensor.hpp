...
struct tensor_idx_view_base {
  ...
  auto dim_data() const -> veg::slice<tensor_dims const> {
...
private:
  ...
  static auto make(i64 begin, i64 end, Fn dims) -> tensor_idx {
    ...
    self.offset_data.resize(veg::narrow<usize>(b.end - b.begin + 1));
    self.dim_data.resize(veg::narrow<usize>(b.end - b.begin));
    ...
    for (i64 t = begin; t < b.end; ++t) {
      ...
      self.dim_data[veg::narrow<usize>(t - begin)] = olr;
      self.offset_data[veg::narrow<usize>(t - begin + 1)] =
          self.offset_data[veg::narrow<usize>(t - begin)] + o * l * r;
  ...
  void assign(tensor_view<value_type const> other) {
    ...
    if (std::is_trivially_copyable<value_type>::value) {
      std::memcpy(
          self.data,
          other.self.data,
          sizeof(value_type) * veg::narrow<usize>(size));
    } else {
...
struct tensor_seq {
  ...
  explicit tensor_seq(idx::tensor_idx idx)
      : self{
            std::vector<T>(veg::narrow<veg::usize>(idx.required_memory())),
            VEG_FWD(idx)} {}
