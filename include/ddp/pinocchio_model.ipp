...
struct pinocchio_impl {
  ...
  static void from_model_geom_builder(
      model<T>& m,
      veg::fn_ref<void(pin::Model&, bool)> builder,
      i64 n_parallel,
    ...
    m.self.data =
        VEG_LAUNDER(static_cast<data_impl*>(boost::alignment::aligned_alloc(
            alignof(data_impl),
            veg::narrow<usize>(m.self.num_data) * sizeof(data_impl))));
    ...
    m.self.model = veg::mem::construct_at(m.self.model, model_double.cast<T>());

    for (i64 i = 0; i < m.self.num_data; ++i) {
      veg::mem::construct_at(
...
model<T>::model(
  ...
  using fn_ref = veg::fn_ref<void(pin::Model&, bool)>;
  internal::pinocchio_impl::from_model_geom_builder(
      *this,
      fn_ref{internal::builder_from_urdf_t{urdf_path}},
      n_parallel,
      add_freeflyer_base);
}
...
auto model<T>::all_joints_test_model(i64 n_parallel) -> model {
  ...
  model m;
  internal::pinocchio_impl::from_model_geom_builder(
      m, fn_ref{&pin::buildAllJointsModel}, n_parallel, false);
  return m;
}
...
model<T>::~model() {
  ...
  if (self.model != nullptr) {
    for (i64 i = 0; i < self.num_data; ++i) {
      veg::mem::destroy_at(self.data + i);
    }
    boost::alignment::aligned_free(self.data);
  }

  if (self.model != nullptr) {
    veg::mem::destroy_at(self.model);
...
void model<T>::neutral_configuration(mut_vec out_q) const {
  ...
  VEG_ASSERT(out_q.rows() == self.config_dim);
...
auto model<T>::frame_classical_acceleration(i64 frame_id, key k) const
  ...
  auto acc = pin::getFrameClassicalAcceleration(
      self.model->pin,
      data->pin,
      veg::narrow<pin::FrameIndex>(frame_id),
...
auto model<T>::frame_se3(i64 frame_id, key k) const -> tuple<key, se3<T>> {
  ...
  auto frame_uid = veg::narrow<veg::usize>(frame_id);
...
auto model<T>::frame_name(i64 i) const -> fmt::string_view {
  ...
  return self.model->pin.frames[veg::narrow<usize>(i)].name;
}
