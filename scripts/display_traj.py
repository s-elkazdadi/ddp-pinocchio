...
if "world/pinocchio" in gui.getNodeList():
    gui.deleteNode("world/pinocchio", True)

L = gui.getNodeList()
for node in L:
    if node.startswith("world/sphere"):
        gui.deleteNode(node, False)
...
gui.setBackgroundColor1(viz.windowID, [1, 1, 1, 1])
gui.setBackgroundColor2(viz.windowID, [1, 1, 1, 1])

with open(sys.argv[1]) as f:
    lines = f.readlines()

if True:
    iterates = [ast.literal_eval(line[5:].strip()) for line in lines]
    ...
    for i, traj in itertools.islice(enumerate(iterates), None, last, step):
        ...
        for x in traj:
            ...
            for k in range(3):
                ...
                if (
                    looking_for[k]
                    and np.linalg.norm(targets[k] - viz.data.oMf[18].translation) < 0.01
                ):
